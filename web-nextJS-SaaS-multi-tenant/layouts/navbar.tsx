    ...
    <>
      <NextUINavbar isBordered maxWidth="full">
        ...
        <NavbarContent as="div" className="w-[50%] lg:mr-[10%]" justify="end">
          <Button isIconOnly variant="bordered" onClick={() => setUploadFormOpen(true)}>
            <UploadIcon />
          </Button>
          <Badge content={reminders.length}>
            <Button isIconOnly variant="bordered" onClick={() => SetReminderModal(true)}>
              <RemindersIcon />
            </Button>
          </Badge>
          {isClient && (
            <Dropdown placement="bottom-end">
              <DropdownTrigger>
                <Avatar
                  ...
                  color="secondary"
                  name={user.userDetails?.firstName?.charAt(0).toLocaleUpperCase()}
                  size="sm"
                  src=""
                />
              </DropdownTrigger>
              <DropdownMenu aria-label="Profile Actions" variant="flat">
                <DropdownItem key="profile" isReadOnly className="h-14" textValue="TEXT">
                  <div className="flex gap-5">
                    <p className="font-semibold">Hey! {user.userDetails?.firstName}</p>
                    <ThemeSwitch />
                  </div>
                </DropdownItem>
                <DropdownItem key="email" isReadOnly className="h-14 gap-1" textValue="TEXT">
                  <p>You are Signed in as</p>
                  <p className="font-semibold">{user.userDetails?.email}</p>
                </DropdownItem>
                <DropdownItem key="update_profile" textValue="TEXT" onClick={onOpenChange}>
                  Update Profile
                </DropdownItem>
                <DropdownItem
                  key="delete"
          ...
          {isOpen && <UserUpdateForm isOpen={isOpen} onOpenChange={onOpenChange} />}
          {uploadFormOpen && (
            <UploadCSVForm isOpen={uploadFormOpen} onOpenChange={setUploadFormOpen} />
          )}
        </NavbarContent>
      </NextUINavbar>
