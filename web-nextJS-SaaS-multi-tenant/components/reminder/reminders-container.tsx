...
import { ModalContent, ModalHeader, ModalBody, Avatar, Spinner } from '@nextui-org/react';
...
export default function RemindersModalContent() {
  const remindersState = useAppSelector((state: RootState) => state.reminders);
      ...
      <ModalBody>
        {remindersState.loading ? (
          <div className="w-[50vh] m-auto">
            <Spinner color="secondary" label="Loading Reminders" labelColor="secondary" />
          </div>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <p className="text-bold text-sm">
                  Reminder: {dayjs(reminderTask.reminder!).fromNow()}
                </p>
                <p className="text-bold text-sm">
                  Deadline: {dayjs(reminderTask.deadline).fromNow()}
                </p>
              </div>
            </div>
          ))
        ...
        <p className="text-center mb-2">Reminders are Updated Every 5 Minutes</p>
      </ModalBody>
    </ModalContent>
  );
}
