  ...
  return (
    ...
    >
      <CardHeader className="flex gap-3">
        ...
        <div className="flex flex-col">
          ...
          <div className="sm:flex-col">
            ...
            {task?.category && (
              <Tooltip content={<div className="w-[100px]">{task.category?.description}</div>}>
      ...
      <div className="flex flex-col ml-[5%]" style={{ fontSize: '12px' }}>
        <Tooltip color="primary" content="Starting Time" offset={1} placement="bottom-start">
          <div className="flex gap-3">
            <CalendarIcon size={18} />
            <p> {moment(task?.startDateTime).format('lll')} </p>
          </div>
        </Tooltip>
        <Tooltip color="danger" content="Deadline" offset={1} placement="bottom-start">
          <div className="flex gap-3">
        ...
        {task?.reminder && (
          <Tooltip color="foreground" content="Reminder" offset={1} placement="bottom-start">
            <div className="flex gap-3">
      ...
      <CardFooter>
        <div style={{ display: 'flex', gap: '20%' }}>
          <Tooltip color="primary" content="Edit Task" offset={1} placement="bottom-start">
            <Button isIconOnly color="warning" onPress={onOpenChange}>
              <EditIcon />
            </Button>
          </Tooltip>
          <Tooltip color="danger" content="Delete Task" offset={1} placement="bottom-start">
            <Button isIconOnly color="danger" onPress={() => setDeletConfirmation(true)}>
              <DeleteIcon />
            </Button>
          </Tooltip>
        </div>
      ...
      {isOpen && <TaskForm isOpen={isOpen} task={task} onOpenChange={onOpenChange} />}
      {deleteConfimation && (
