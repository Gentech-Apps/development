      ...
      case 'priority':
        return (
          <div className="flex flex-col">
            ...
            <p className="text-bold text-sm capitalize">{moment(task.deadline).format('lll')}</p>
          </div>
        );
      case 'status':
        return (
        ...
        return (
          <div className="relative flex items-center gap-2">
            <Tooltip content="Edit Task">
              <Button isIconOnly color="warning" onPress={() => handleEdit(task)}>
                <EditIcon />
              </Button>
            </Tooltip>
            <Tooltip color="danger" content="Delete Task">
              <Button isIconOnly color="danger" onPress={() => handleDelete(task)}>
                <DeleteIcon />
              </Button>
            </Tooltip>
          </div>
  ...
  return taskState.loading ? (
    ...
    <>
      <Table aria-label="task-table" className="px-3">
        ...
        {
          <TableBody emptyContent="No tasks, Start Creating..." items={taskState.tasks}>
            {(item) => (
      ...
      {isOpen && <TaskForm isOpen={isOpen} task={editingTask} onOpenChange={onOpenChange} />}
      {deleteConfimation && (
