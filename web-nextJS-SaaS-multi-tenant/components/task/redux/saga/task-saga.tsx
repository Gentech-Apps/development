...
import { setError, setSuccess, setWarnings } from '@/components/common/slices/information-slice';
...
export function* handleTaskCreation(
  action: PayloadAction<{ createTask: CreateTask; params: string }>,
) {
  const token = getToken();
...
export function* handleTaskUpdation(action: PayloadAction<{ id: number; formData: CreateTask }>) {
  const token = getToken();
  const data: Task = yield apiCall<Task>({
    route: `api/task/${action.payload.id}`,
    method: 'PUT',
...
export function* handleImportCSV(action: PayloadAction<{ file: File; params: string }>) {
  ...
  try {
    ...
    if (response.ok) {
      const data: { tasks: Task[]; errors: Map<string, string> } = yield response.json();

      yield put(filterTasks(action.payload.params));
      if (Object.values(data.errors).length > 0) {
        yield put(setWarnings(data.errors as unknown as Record<string, string>));
    ...
    const errorMessage = (error as Error).message ?? 'An error occurred while Creating Task.';
...
export function* handleDownloadCSV(action: PayloadAction<{ params: string; pageNo: number }>) {
  try {
    const token = getToken();
    ...
    const errorMessage = (error as Error).message ?? 'An error occurred while fetching tasks.';
