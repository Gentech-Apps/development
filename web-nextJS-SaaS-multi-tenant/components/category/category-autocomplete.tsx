...
export const CategoryAutoComplete: React.FC<CategoryAutoCompleteProps> = ({
  ...
  const handleChange = (selectedKey: string | null) => {
    if (selectedKey) {
      const selectedCategory = categories.find((category) => category.id === +selectedKey);
  ...
  return (
    <div className="flex w-full flex-wrap md:flex-nowrap gap-4">
      <Autocomplete
        ...
        onSelectionChange={(selectedKey) => handleChange(selectedKey?.toString() || null)}
      >
        {categories.map((category) => (
          <AutocompleteItem key={category.id} textValue={category.name} value={category.id}>
            {category.name}
          </AutocompleteItem>
        ))}
      </Autocomplete>
