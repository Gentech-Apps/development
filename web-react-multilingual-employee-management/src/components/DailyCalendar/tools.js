...
class DailyCalendarTools {
  static createScheduleForUser = (processes, isFirstProcess, isLastProcess) => {
    ...
    if (!processes.length) {
      return [
        <Downtime
          duration={10 * MILISECONDS_PER_HOUR}
          key={uuid()}
    ...
    processes.forEach((order_process, index) => {
      ...
      const processStartTime =
        this.getDecimalFormattedStartTime(start_time) ||
        (endHour < START_OF_WORKING_DAY ? START_OF_WORKING_DAY : endHour);
...
export default DailyCalendarTools;
